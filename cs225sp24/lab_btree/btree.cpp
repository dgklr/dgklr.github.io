...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if (first_larger_idx < order && subroot->elements[first_larger_idx] == key) return subroot->elements[first_larger_idx].value;
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //std::cout << child->elements.size() << ' ' << child->children.size() << ' ' << mid_elem_idx << ' ' << mid_child_idx << std::endl;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (subroot->is_leaf) {
        if (subroot->children.size() == 0) subroot->children.push_back(nullptr);
        subroot->elements.insert(subroot->elements.begin()+first_larger_idx, pair);
        subroot->children.push_back(nullptr);
        return;
    }
    ...
    //assert(subroot->children[first_larger_idx]->elements.size() <= order);

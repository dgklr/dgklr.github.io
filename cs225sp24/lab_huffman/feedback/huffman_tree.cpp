...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (auto i : frequencies)
        singleQueue.push(new TreeNode(i));
    while (singleQueue.size() + mergeQueue.size() > 1) {
        TreeNode* tmp1 = this->removeSmallest(singleQueue, mergeQueue);
        TreeNode* tmp2 = this->removeSmallest(singleQueue, mergeQueue);
        TreeNode* tmp = new TreeNode(tmp1->freq.getFrequency() + tmp2->freq.getFrequency());
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit()) {
        ...
        retval->left = readTree(bfile);
        retval->right = readTree(bfile);
        return retval;
    }
}
